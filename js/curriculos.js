    ...
    listDiv.innerHTML = `
        ...
        <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden">
            <div class="p-4 border-b border-slate-200 flex gap-4 bg-slate-50 dark:bg-slate-900 dark:border-slate-700">
                <input type="text" id="busca-curriculos" onkeyup="filtrarCurriculos()" placeholder="Buscar nome, cargo..." class="flex-1 input-field max-w-md">
                ...
                <table class="w-full text-left text-sm text-slate-600 dark:text-slate-300">
                    <thead class="bg-slate-50 dark:bg-slate-900 text-slate-700 dark:text-slate-200 uppercase font-bold text-xs border-b border-slate-200 dark:border-slate-700">
                        <tr>
                            ...
                            <th class="px-6 py-4">Cargo Proposto</th>
                            <th class="px-6 py-4">Indicação</th>
                            <th class="px-6 py-4">Status / Local</th>
                            <th class="px-6 py-4 text-center">Anexo</th>
                            <th class="px-6 py-4 text-right">Ações</th>
                        </tr>
                    </thead>
                    <tbody id="tbody-curriculos" class="divide-y divide-slate-100 dark:divide-slate-700">
                        <tr><td colspan="7" class="text-center p-8 text-slate-400">Carregando currículos...</td></tr>
                    </tbody>
                </table>
            </div>
        </div>
...
async function carregarCurriculos() {
    try {
        ...
        todosCurriculos = [];
        snap.forEach(doc => {
            todosCurriculos.push({ id: doc.id, ...doc.data() });
        });
...
function renderCurriculos() {
    ...
    todosCurriculos.forEach(c => {
        const text = `${c.nome || ''} ${c.cargo_proposto || ''} ${c.indicacao || ''}`.toLowerCase();
        ...
        html += `
            <tr class="hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">
                <td class="px-6 py-3 text-sm">${dateFmt}</td>
                <td class="px-6 py-3 font-bold text-slate-700 dark:text-slate-200 uppercase">${c.nome}<div class="text-xs font-normal text-slate-400 mt-0.5"><i data-lucide="phone" class="w-3 h-3 inline mr-1"></i>${c.telefone||'-'}</div></td>
                <td class="px-6 py-3 font-bold text-slate-600 dark:text-slate-300 text-xs">${c.cargo_proposto}</td>
                <td class="px-6 py-3 text-xs uppercase">${c.indicacao||'-'}</td>
                <td class="px-6 py-3"><span class="px-2 py-1 rounded-md text-[10px] font-bold uppercase ${badge}">${c.status}</span>${localContratado}</td>
                <td class="px-6 py-3 text-center">${linkAnexo}</td>
                <td class="px-6 py-3 text-right space-x-2">
                    <button onclick="editarCurriculo('${c.id}')" class="text-purple-500 hover:text-purple-700 bg-purple-50 dark:bg-purple-900/30 p-1.5 rounded transition" title="Editar / Ver Detalhes"><i data-lucide="edit-3" class="w-4 h-4"></i></button>
    ...
    if (count === 0) {
        tbody.innerHTML = `<tr><td colspan="7" class="text-center p-6 text-slate-400">Nenhum currículo encontrado.</td></tr>`;
    } else {
...
async function uploadArquivoCurriculo() {
    ...
    if(!file) {
        window.showModalAlert("Selecione um arquivo primeiro.");
        return;
    }
    
    if (file.size > 5 * 1024 * 1024) {
        window.showModalAlert('O arquivo deve ter no máximo 5MB.');
        return;
    }
...
document.addEventListener("DOMContentLoaded", () => {
    setTimeout(initCurriculos, 1700);
});
    ...
    if (typeof todosPacientes !== 'undefined' && Array.isArray(todosPacientes) && todosPacientes.length > 0) {
        ...
        if (encontrados.length > 0 && encontrados.length <= 15) {
            ...
            encontrados.forEach(p => {
                ...
                listaHtml += `<div onclick="selecionarMunicipeCurriculo(${pJson})" class="px-4 py-2 hover:bg-purple-50 cursor-pointer border-b border-slate-100 flex flex-col text-left">
...
window.selecionarMunicipeCurriculo = function(p) {
    ...
    // Auto fill telephone if available
    if(p.tel) {
        document.getElementById('curriculo_telefone').value = p.tel;
    }
