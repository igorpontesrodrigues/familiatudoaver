                ...
                <table class="w-full text-left text-sm text-slate-600 dark:text-slate-300">
                    <thead class="bg-slate-50 dark:bg-slate-900 text-slate-700 dark:text-slate-200 uppercase font-bold text-xs border-b border-slate-200 dark:border-slate-700">
                        <tr>
                            ...
                            <th class="px-6 py-4">Munícipe</th>
                            <th class="px-6 py-4">Serviço Solicitado</th>
                            <th class="px-6 py-4">Endereço/Local</th>
                            <th class="px-6 py-4">Status</th>
                            <th class="px-6 py-4 text-right">Ações</th>
                        </tr>
                    </thead>
                    <tbody id="tbody-servicos" class="divide-y divide-slate-100 dark:divide-slate-700">
                        <tr><td colspan="6" class="text-center p-8 text-slate-400">Carregando serviços...</td></tr>
                    </tbody>
                </table>
            </div>
        </div>
...
function renderServicosPublicos() {
    ...
    todosServicos.forEach(s => {
        const text = `${s.nome_municipe || ''} ${s.endereco || ''} ${s.tipo_servico || ''}`.toLowerCase();
        ...
        html += `
            <tr class="hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">
                ...
                <td class="px-6 py-3 font-bold text-slate-700 dark:text-slate-200">${s.nome_municipe}</td>
                <td class="px-6 py-3"><span class="font-bold text-indigo-700 dark:text-indigo-400 text-xs">${s.tipo_servico}</span></td>
                <td class="px-6 py-3 text-xs text-slate-500 dark:text-slate-400">${s.endereco}</td>
                <td class="px-6 py-3"><span class="px-2 py-1 rounded-md text-[10px] font-bold uppercase ${badge}">${s.status}</span></td>
                <td class="px-6 py-3 text-right space-x-2">
                    <button onclick="editarServico('${s.id}')" class="text-indigo-500 hover:text-indigo-700 bg-indigo-50 dark:bg-indigo-900/30 dark:hover:bg-indigo-900/50 p-1.5 rounded transition" title="Editar"><i data-lucide="edit-2" class="w-4 h-4"></i></button>
                    <button onclick="deletarServico('${s.id}')" class="text-rose-500 hover:text-rose-700 bg-rose-50 dark:bg-rose-900/30 dark:hover:bg-rose-900/50 p-1.5 rounded transition" title="Excluir"><i data-lucide="trash-2" class="w-4 h-4"></i></button>
                </td>
            </tr>
        `;
    });
    ...
    if (count === 0) {
        tbody.innerHTML = `<tr><td colspan="6" class="text-center p-6 text-slate-400">Nenhum serviço encontrado.</td></tr>`;
    } else {
